...
* `dnd-auto-scroll` Optional boolean expression. When it evaluates to true, the list will automatically scroll up and down (or left and right if `dnd-horizontal-list` is set to true) when an element is being dragged over. When the element approaches the first quarter of the window it will scroll up (or left). When the element approaches the last quarter of the window it will scroll down (or right). Same mechanics is done if the list has predefined height and the `overflow` is set to `auto` or `scroll`.
